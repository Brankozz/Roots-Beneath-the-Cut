...
things = []
with open(os.path.join('/home/cizhang/concept-prune/datasets', 'things.txt'), 'r') as f:
    things = f.readlines()
things = [thing.strip() for thing in things]

humans = []
with open(os.path.join('/home/cizhang/concept-prune/datasets', 'humans.txt'), 'r') as f:
    humans = f.readlines()
humans = [human.strip() for human in humans]

common_scenes = []
with open(os.path.join('/home/cizhang/concept-prune/datasets', 'common_scenes.txt'), 'r') as f:
...
scenes_church = ['on a road', 'in the park', 'near the beach', ' on the street', 'in front of a house',
                 'in front of a statue', 'near a tree', 'in a forest']

places_names = ['road', 'park', 'street', 'white car', 'blue car', 'black car', 'white automobile', 'black automobile',
                'blue automobile', 'truck', 'tree', 'forest', 'house']
places_truck = ['on the road', 'in the park', 'on the street', 'near a white car', 'near a blue car',
                'near a black car', 'near a white automobile', 'near a black automobile', 'near a blue automobile',
                'near a big truck', 'near a tree', 'in a forest', 'in front of a house']

helmet_scenes_names = ['table', 'road', 'locker', 'box', 'wall', 'window', 'chair', 'desk', 'backpack', 'motorcycle',
                       'mirror', 'door']
helmet_scenes = ['on a table', 'on the road', 'in the locker', 'in a box', 'near the wall', 'near a window',
                 'under a chair', 'under a desk', 'beside a backpack', 'beside a motorcycle', 'in front of a mirror',
                 'in front of a door']

spider_scenes_names = ['wall', 'park', 'street', 'web', 'tree', 'flower', 'leaf', 'window', 'rock', 'branch', 'door',
                       'curtain']
spider_scenes = ['on the wall', 'in the park', 'on the street', 'on a web', 'on a tree', 'on a flower', 'on a leaf',
                 'near a window', 'on a rock', 'on a branch', 'near a door', 'near a curtain']
...
def get_prompts(args):
    ...
    if target_type == 'naked':
        ...
        if target in ["parachute", "cassette player", "golf ball", "gas pump", "english springer", "tench", "chain saw",
                      "french horn", "television", "helmet", "squirrel", "spider", "bicycle", "traffic light"]:
            base_prompts = [f'a {thing}' for thing in common_scenes]
            target_prompts = [f'a {target} in a {thing}' for thing in common_scenes]

        # elif target in ['helmet']:
        #     base_prompts = [f'a {thing}' for thing in helmet_scenes_names]
        #     target_prompts = [f'a {target} {thing}' for thing in helmet_scenes]

        # elif target in ['spider']:
        #     base_prompts = [f'a {thing}' for thing in spider_scenes_names]
        #     target_prompts = [f'a {target} {thing}' for thing in spider_scenes]
