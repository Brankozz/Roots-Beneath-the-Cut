...
def input_args():
    ...
    parser.add_argument('--target', type=str, default='garbage truck')
    ...
    parser.add_argument('--ckpt_name', type=str, default='/home/cizhang/Documents/Ci/concept_prune/wanda/results_Ci/results_seed_0/stable-diffusion/runwayml/stable-diffusion-v1-5/tench/model/filled_with_mag_seed2021.pt')
...
class CustomDatasetErasure(torch.utils.data.Dataset):
    def __init__(self, data, concepts_to_remove):
        ...
        # print("concepts_to_remove:", concepts_to_remove)
        # print("unique labels:", set([l.lower() for l in self.labels]))
...
def main():
    ...
    if args.removal_mode == 'erase':
        dataloader = torch.utils.data.DataLoader(CustomDatasetErasure(data, args.target), batch_size=args.batch_size, shuffle=False, num_workers=16)
    else:
        dataloader = torch.utils.data.DataLoader(CustomDatasetKeep(data, args.target), batch_size=args.batch_size, shuffle=False, num_workers=16)
    ...
    avg_acc = 0
    # img_save_path = os.path.join('/scratch/cz06540/concept_prune_image_chence', args.target, f'recover_{args.top_ratio}_Sample')
    # os.makedirs(img_save_path, exist_ok=True)
    # os.makedirs(os.path.join(f'/scratch/cz06540/concept-prune-image/{args.target}', f'Preserve_Eva_with_top_ratio{args.top_ratio}'), exist_ok=True)
    for iter, (prompt, seed, label) in enumerate(dataloader):
        ...
        np.random.seed(int(seed[0].item()))
        removal_images = remover_model(prompt).images
        # save images
        # for i, image in enumerate(removal_images):
            # image.save(os.path.join(f'/scratch/cz06540/concept-prune-image/{args.target}', f'Preserve_Eva_with_top_ratio{args.top_ratio}', f"{iter * args.batch_size + i}.png"))
            # image.save(os.path.join('/scratch/cz06540/concept_prune_image_chence', args.target, f'recover_{args.top_ratio}_Sample', f"{iter * args.batch_size + i}.png"))
    ...
    # save_dir = f"./Related_Acc_Eva_Unified_on_A100/Recovered_AVE/{args.top_ratio}"
    # os.makedirs(save_dir, exist_ok=True)
    # with open(f"./Related_Acc_Eva_Unified_on_A100/Recovered_AVE/{args.top_ratio}/{args.target}_v1-5.json", "w") as f:
