...
def input_args():
    ...
    parser.add_argument('--top_ratio', type=int, default=0.8)
...
def main():
    ...
    print("Benchmarking result path: ", args.benchmarking_result_path)
    if not os.path.exists(f'{args.benchmarking_result_path}/{args.top_ratio}'):
        os.makedirs(f'{args.benchmarking_result_path}/{args.top_ratio}')

    # make a dataloader of prompts
    dataloader = torch.utils.data.DataLoader(prompts, batch_size=1, shuffle=False)

    # Original SD model
    model = StableDiffusionPipeline.from_pretrained(args.model_id, torch_dtype=torch.float16, safety_checker=None, requires_safety_checker=False)
    ...
    for i, prompt in enumerate(dataloader):
        ...
        if os.path.exists(os.path.join(f'{args.benchmarking_result_path}/{args.top_ratio}', f"{i}_original.png")):
            ...
            # save imagesf'{args.benchmarking_result_path}/{args.top_ratio}'
            original_image.save(os.path.join(f'{args.benchmarking_result_path}/{args.top_ratio}', f"{i}_original.png"))
            removal_image.save(os.path.join(f'{args.benchmarking_result_path}/{args.top_ratio}', f"{i}_removed.png"))
    ...
    save_dir = f"./Nudity_Eva_Unified_on_A100/Recovered/top_{args.top_ratio}"
    os.makedirs(save_dir, exist_ok=True)
    with open(f"./Nudity_Eva_Unified_on_A100/Recovered/top_{args.top_ratio}/{args.target}_v1-5.json", "w") as f:
